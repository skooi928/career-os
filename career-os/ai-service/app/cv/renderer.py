...
# Renders CVData into a clean, single-column, ATS-optimised HTML document.
...
#   - Unicode-safe characters only
#   - Clear date formatting
#   - Font: system sans-serif (safe for ATS text extraction)
# """
...
#     max-width: 250px;         /* Optional: Limits width so one long link doesn't push others away */
...
#   .lang-level { color: #6b6b6b; }

#   /* ── Awards / Certs / Activities ─────────────── */
#   .list-item {
#     margin-bottom: 6px;
#     font-size: 9.5pt;
#     line-height: 1.45;
...
#   .list-item-title { font-weight: 600; color: #1a1a1a; }
#   .list-item-meta { color: #6b6b6b; font-size: 9pt; margin-top: 1px; }

#   /* ── References ──────────────────────────────── */
#   .ref-item { margin-bottom: 8px; font-size: 9.5pt; page-break-inside: avoid; }
#   .ref-name { font-weight: 700; color: #1a1a1a; }
#   .ref-role { color: #444444; }
#   .ref-contact { color: #009161; }
...
#         {%- if exp.is_current %}Present{%- elif exp.end_date %}{{ exp.end_date }}{% endif %}
#       </div>
#     </div>
#     {%- if exp.company %}
...
#       {%- if proj.link %}
#       <div class="item-date">{{ proj.link }}</div>
#       {%- endif %}
#     </div>
...
#     """Render CVData into ATS-optimised HTML string."""
...
Renders CVData into a clean, single-column, ATS-optimised HTML document
for xhtml2pdf conversion.

Design improvements applied:
  - Georgia (serif) for headings, Arial for body — clear hierarchy
  - Only section headings use brand colour (#009161) and uppercase
  - Job titles, company names, institution names use black/dark grey only
  - Generous spacing between sections (margin-bottom: 18px)
  - URLs kept on one line with overflow-wrap and max-width constraint
  - Skills support optional category grouping
  - Float-based two-column layout for item headers (xhtml2pdf safe)

ATS principles maintained:
  - Single column layout
  - Selectable text throughout
  - No images, icons, or decorative graphics
  - Standard section heading labels
