        ...
        <div class="filters-column">
          <div class="card p-4 filter-card">
            ...
            <div class="filter-section mt-4">
              <h4>Location</h4>
              <div class="filter-options">
                <div class="search-bar" style="padding: 6px 12px; box-shadow: none; border: 1px solid var(--color-border); border-radius: 8px;">
                  <i class="ph ph-map-pin" style="font-size: 1.125rem; color: var(--color-text-secondary); margin-left: 0;"></i>
                  <input type="text" [(ngModel)]="locationFilter" (ngModelChange)="onLocationFilterChange()" placeholder="e.g. Selangor..." style="font-size: 0.9375rem; padding: 4px 0;">
                </div>
              </div>
            </div>
          </div>
    ...
    this.filteredJobs$ = combineLatest([
      ...
      map(([jobs, query]) => {
        ...
        if (this.locationFilter.trim()) {
           const lowercaseLoc = this.locationFilter.trim().toLowerCase();
           filtered = filtered.filter(job => job.location?.toLowerCase().includes(lowercaseLoc));
        }
