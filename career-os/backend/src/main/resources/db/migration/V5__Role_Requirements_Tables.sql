...
-- Create Role Technical Skill Requirement table
CREATE TABLE IF NOT EXISTS public.role_technical_skill_requirements (
  role_technical_skill_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  requirement_id UUID NOT NULL REFERENCES public.role_requirements(requirement_id) ON DELETE CASCADE,
  technical_skill_text VARCHAR(255) NOT NULL
);

-- Create Must Have Requirements table
CREATE TABLE IF NOT EXISTS public.role_must_have_requirements (
  must_have_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  requirement_id UUID NOT NULL REFERENCES public.role_requirements(requirement_id) ON DELETE CASCADE,
